...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    epcf = ngx_event_get_conf(cycle->conf_ctx, ngx_epoll_module);  //获得epoll配置 ngx_epoll_conf_t

    if (ep == -1) {
        ep = epoll_create(cycle->connection_n / 2); //创建epool描述符
    ...
    ngx_event_actions = ngx_epoll_module_ctx.actions;  //设置 ngx_event_actions    ngx_epoll_module_ctx
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    c = ev->data;  //获得connection对象

    events = (uint32_t) event;

    if (event == NGX_READ_EVENT) { // 读事件
    	//因为需要确定下是否有写事件也在epoll里，来决定到底增加事件还是修改
        e = c->write;
        prev = EPOLLOUT; //前一个事件，写事件
#if (NGX_READ_EVENT != EPOLLIN|EPOLLRDHUP)
        events = EPOLLIN|EPOLLRDHUP; //EPOLLRDHUP 事件，代表对端断开连接
#endif

    } else { //写事件
        e = c->read;
        prev = EPOLLIN|EPOLLRDHUP; //前一个事件，读事件
#if (NGX_WRITE_EVENT != EPOLLOUT)
        events = EPOLLOUT;
#endif
    }

    if (e->active) { //在event传入read的情况下，e为write，如果write早已经加入epoll了，那这时候需要把read也加进去，需要modify一下
        op = EPOLL_CTL_MOD;
        events |= prev; //叠加上事件

    } else {
        op = EPOLL_CTL_ADD; //否则只是单纯的增加一个事件
    }

#if (NGX_HAVE_EPOLLEXCLUSIVE && NGX_HAVE_EPOLLRDHUP)
	//内核4.5+开始新增的水平触发模式的EPOLLEXCLUSIVE标志，
	//这个标志会保证一个事件只有一个epoll_wait()会被唤醒，避免了"惊群效应"
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) { //如果启用二零post的话，就放入队列，后续处理
                ...
                rev->handler(rev);  //否则直接调用事件的handle
            }
        }
