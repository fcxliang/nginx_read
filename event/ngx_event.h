...
struct ngx_event_s {
    void            *data;  //如果core的指向connection，如果是envent

    unsigned         write:1;

    unsigned         accept:1;  //等待accept请求
    ...
    ngx_event_handler_pt  handler; //  ngx_event_recvmsg ngx_event_accept
...
#define NGX_READ_EVENT     (EPOLLIN|EPOLLRDHUP) //有数据到达或对端发送FIN或本端关闭读
...
typedef struct {
    ...
    ngx_uint_t    use;  //use 指令: select/poll/kqueue(bsd)/epoll/eventport(solaris)

    ngx_flag_t    multi_accept;
    ngx_flag_t    accept_mutex; //本来是要解决惊群问题的,现在内核4.5以上支持EPOLLEXCLUSIVE,epoll惊群解决了
