...
static ngx_log_t        ngx_log;   //日志结构
static ngx_open_file_t  ngx_log_file; //打开的日志文件描述符
...
ngx_log_stderr(ngx_err_t err, const char *fmt, ...)
{
    u_char   *p, *last;
    va_list   args;
    u_char    errstr[NGX_MAX_ERROR_STR];  //2k的数组

    last = errstr + NGX_MAX_ERROR_STR;  //数组尾指针

    p = ngx_cpymem(errstr, "nginx: ", 7); // ngx_cpymem拷贝完后，指针指向新字符串之后的地址
...
/*
	日志名是固定的名字logs/error.log
	这个函数实现有点混乱
	1. 首先使用宏定义死了日志文件的名称，logs/error.log
	2. 代码其实支持额外定义的日志名称，而且如果名称是全路径则使用全路径，
	   如果是相对路径则前面追加-p的路径或者使用默认prefix
...
ngx_log_init(u_char *prefix)
{
    ...
    if (nlen == 0) {
        ngx_log_file.fd = ngx_stderr; //没有名称的话，输出到标准出错
        return &ngx_log;
    }
    ...
    if (name[1] != ':') {
#else
    if (name[0] != '/') { //不以/开头说明不是全路径
#endif

        if (prefix) { //是否指定了prefix，没有指定就用默认的
        ...
        if (plen) {
            name = malloc(plen + nlen + 2); //为name分配全路径看空间，perfix+name
            if (name == NULL) {
                return NULL;
            }

            p = ngx_cpymem(name, prefix, plen); //prefix复制到name，返回当前游标，指向下一个

            if (!ngx_path_separator(*(p - 1))) { //如果末尾没有/就加上
                *p++ = '/';
            }

            ngx_cpystrn(p, (u_char *) NGX_ERROR_LOG_PATH, nlen + 1); //name = prefix+err log path 再把相对路径复制过去就是全路径名了
    ...
    if (ngx_log_file.fd == NGX_INVALID_FILE) {
        ...
        ngx_log_file.fd = ngx_stderr; //如果打开日志文件失败的话，日志文件为标准出错
    }

    if (p) {
        ngx_free(p); //释放掉名字， 文件打开了，把文件名的空间释放掉，并没有保存，可能下次打开还要重新计算名字
    }
...
ngx_log_open_default(ngx_cycle_t *cycle)
{
    ...
    log->file = ngx_conf_open_file(cycle, &error_log); //创建其一个errorlog
    if (log->file == NULL) {
        return NGX_ERROR;
    }
...
ngx_log_set_log(ngx_conf_t *cf, ngx_log_t **head)
{
    ...
    if (ngx_strcmp(value[1].data, "stderr") == 0) {  //------------------------>stderr
        ngx_str_null(&name);
    ...
    } else if (ngx_strncmp(value[1].data, "memory:", 7) == 0) { //------------->memory  DEBUG
...
#else
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "nginx was built without debug support");  //非debug模式，不能使用memory buffer
        return NGX_CONF_ERROR;
#endif

    } else if (ngx_strncmp(value[1].data, "syslog:", 7) == 0) { //---------------->syslog
        ...
        new_log->file = ngx_conf_open_file(cf->cycle, &value[1]);  //------------------------------------------->指定配置文件
        if (new_log->file == NULL) {
            return NGX_CONF_ERROR;
        }
    }
