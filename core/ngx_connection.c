...
/*
	cycle->listening.elts[x].fd记录了旧有的socket fd
	现在通过获取每个fd的socket属性，重新填充cycle->listening.elts[x]的其他字段
*/
ngx_int_t
ngx_set_inherited_sockets(ngx_cycle_t *cycle) //从继承的socket上把相关信息取下来
{
    size_t                     len;
    ngx_uint_t                 i;
    ngx_listening_t           *ls;
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        ls[i].sockaddr = ngx_palloc(cycle->pool, sizeof(ngx_sockaddr_t)); //分配存放地址的空间
        if (ls[i].sockaddr == NULL) {
            return NGX_ERROR;
        }

        ls[i].socklen = sizeof(ngx_sockaddr_t);
        if (getsockname(ls[i].fd, ls[i].sockaddr, &ls[i].socklen) == -1) { //获得本端socket信息sockaddr
        ...
        ls[i].addr_text.data = ngx_pnalloc(cycle->pool, len); //为IP:端口这样的文字描述分配空间
        if (ls[i].addr_text.data == NULL) {
            return NGX_ERROR;
        }

        len = ngx_sock_ntop(ls[i].sockaddr, ls[i].socklen,
                            ls[i].addr_text.data, len, 1);  //地址字符串写进data，返回实际的字符串长度
        if (len == 0) {
            return NGX_ERROR;
        }

        ls[i].addr_text.len = len; //字符串地址长度

        ls[i].backlog = NGX_LISTEN_BACKLOG;

        olen = sizeof(int);

        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_TYPE, (void *) &ls[i].type, //获得socket类型，例如：SOCK_STREAM，SOCK_DGRAM
                       &olen)
            == -1)
        {
            ngx_log_error(NGX_LOG_CRIT, cycle->log, ngx_socket_errno,
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_RCVBUF, (void *) &ls[i].rcvbuf, //获得原始缓接收冲区的大小
                       &olen)
            == -1)
        {
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_SNDBUF, (void *) &ls[i].sndbuf, //获得原始发送缓冲区的大小
                       &olen)
            == -1)
        {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
...
#if (NGX_HAVE_REUSEPORT)  //多个进程绑定同一个端口

        reuseport = 0;
        olen = sizeof(int);

#ifdef SO_REUSEPORT_LB //支持负载均衡
...
#if (NGX_HAVE_DEFERRED_ACCEPT && defined TCP_DEFER_ACCEPT) //支持省略客户端ack，即accept在收到第一个数据包时返回
...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            if (ls[i].add_reuseport) {  //新启动的不会有这个，否
            ...
            if (ls[i].fd != (ngx_socket_t) -1) {  //新启动的nginx都是-1
                continue;
            }

            if (ls[i].inherited) { //继承的这种也不管
...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    if (ngx_cycle->files && (ngx_uint_t) s >= ngx_cycle->files_n) {  //socket大于最大打开文件描述符肯定是不对的
    ...
    if (ngx_cycle->files && ngx_cycle->files[s] == NULL) {
        ngx_cycle->files[s] = c; //把connection对象放入files中对应的空间，文件描述符为索引
    }

    rev = c->read; //read事件
    wev = c->write; //write事件

    ngx_memzero(c, sizeof(ngx_connection_t)); //清零
