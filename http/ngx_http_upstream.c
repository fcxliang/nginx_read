...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
	...
	//snat pool test test test!!! by lxl
	if (ngx_http_upstream_snat(r, u, u->conf->snat_pool) != NGX_OK) {
		ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
	}
...
// add by lxl, for snatpool
char *
ngx_http_upstream_snat_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    char *p = conf;
    ngx_str_t                  *value;
    ngx_array_t               **psnatpool, *snatpool;	
	
    int                         nadds 		= 0;
    int                         len 		= 0;
    unsigned char              *paddrs 		= NULL;
    int                         cur, start 	= 0;
    int                         naddr 		= 0;


    psnatpool = (ngx_array_t **) (p + cmd->offset);
	
    if (*psnatpool != NGX_CONF_UNSET_PTR) {
        return "is duplicate";
    }
    ...
    for (cur = start + 1; cur <= len; cur ++) {
        if (paddrs[cur] == ',' || cur == len) {
            ...
            ngx_memcpy(paddr, &paddrs[start], addr_size);

            local = ngx_array_push(snatpool);
            if (local == NULL) {
                return NGX_CONF_ERROR;
            }
            ...
            rc = ngx_parse_addr_port(cf->pool, local->addr, paddr, addr_size);

            switch (rc) {
            case NGX_OK:
                local->addr->name.len = addr_size;
                local->addr->name.data = paddr;
                break;
...
static ngx_int_t ngx_http_upstream_snat(ngx_http_request_t *r, ngx_http_upstream_t *u, ngx_array_t *snat_pool) {
    ...
    if (snat_pool == NGX_CONF_UNSET_PTR)
    {
        return NGX_OK;
    }

    chose = ngx_random() % snat_pool->nelts;

    local = &((ngx_http_upstream_local_t *)snat_pool->elts)[chose];
    //local = (ngx_http_upstream_local_t *) &(snat_pool->elts[chose]);
