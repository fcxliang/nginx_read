    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (us->servers) {
        ...
        for (i = 0; i < us->servers->nelts; i++) { //遍历servers数组
            if (server[i].backup) {
                continue;
            }
        ...
        us->peer.data = peers; //原来peer.data就是peers！
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    rrp = r->upstream->peer.data; // 比如sticky的peer data
    ...
    if (rrp->peers->next && rrp->peers->next->number > n) { //下一个元素
    ...
    ngx_http_upstream_resolved_t *ur)
{
    ...
    peers = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_rr_peers_t));  //创建peers
    if (peers == NULL) {
        return NGX_ERROR;
    }

    peer = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_rr_peer_t)  // 创建n个peer的数组
                                * ur->naddrs);
    if (peer == NULL) {
        return NGX_ERROR;
    }

    peers->single = (ur->naddrs == 1); //单peer
    peers->number = ur->naddrs; //peer数量
    peers->name = &ur->host; // ？？给upstream的名字赋值？

    if (ur->sockaddr) {
        peer[0].sockaddr = ur->sockaddr;
        peer[0].socklen = ur->socklen;
        peer[0].name = ur->name.data ? ur->name : ur->host; //TODO: 可能是upstream名称或者是一个域名?
        peer[0].weight = 1;
        ...
        for (i = 0; i < ur->naddrs; i++) { //dns解出多个IP地址？每个IP一个peer
    ...
    if (rrp->peers->number <= 8 * sizeof(uintptr_t)) { //64 on 64bits os， 看起来是一组8个
        rrp->tried = &rrp->data;
        rrp->data = 0;

    } else { // > 64
        n = (rrp->peers->number + (8 * sizeof(uintptr_t) - 1)) // 算出需要几组指针
                / (8 * sizeof(uintptr_t));
...
ngx_http_upstream_get_round_robin_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (peers->single) { //单peer
        peer = peers->peer;

        if (peer->down) { //是否是down的
            goto failed;
        }
        ...
        rrp->current = peer;  //选中的peer

    } else { //多peer，需要round robin
