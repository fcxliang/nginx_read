...
static ngx_command_t ngx_http_vhost_traffic_status_commands[] = {
    ...
    { ngx_string("vhost_traffic_status_display_sum_key"),  //默认是*
      NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
...
// holyzone
ngx_msec_int_t
ngx_http_vhost_traffic_status_upstream_header_time(ngx_http_request_t *r)
{
    ngx_uint_t                  i;
    ngx_msec_int_t              ms;
...
ngx_http_vhost_traffic_status_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    size = NGX_HTTP_VHOST_TRAFFIC_STATUS_DEFAULT_SHM_SIZE;  //1M

    for (i = 1; i < cf->args->nelts; i++) {
        if (ngx_strncmp(value[i].data, "shared:", 7) == 0) {  //vhost_traffic_status_zone [shared:name:size] 例如 shared:vhost_traffic_status:1m

            name.data = value[i].data + 7;

            p = (u_char *) ngx_strchr(name.data, ':'); // 取name
            if (p == NULL) {
            ...
            size = ngx_parse_size(&s);  //转成具体的字节大小
            if (size == NGX_ERROR) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                                   "invalid shared size \"%V\"", &value[i]);
                return NGX_CONF_ERROR;
            ...
            if (size < (ssize_t) (8 * ngx_pagesize)) {  //必须大于页的8倍
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
    ...
    shm_zone->data = ctx;  // ctx指针放在data上
...
ngx_http_vhost_traffic_status_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->filter_keys == NULL) {
        ...
        if (conf->filter_check_duplicate != 0) {
            // lxl beigin
            //需要合并计数
            //此时我们需要 合并继承，如果父子filterkey不一致需要同时继承父key
            ngx_array_t  *merged_keys;
            if (prev->filter_keys != NULL) {
                rc = ngx_http_vhost_traffic_status_filter_merge(cf->pool, &merged_keys, conf->filter_keys, prev->filter_keys);
                if (rc != NGX_OK) {
                    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "merge srv & loc filters failed, lxl");
                    return NGX_CONF_ERROR;
                }
                // ngx_array_destroy(conf->filter_keys); //vts也不释放内存，我也不释放了 需要注意！
                conf->filter_keys = merged_keys;
            }
