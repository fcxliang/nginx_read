    ...
    ngx_http_upstream_server_t *us)
{
	...
	//holyzone
    else if (ngx_http_vhost_traffic_status_node_member_cmp(member, "RequestCurrent") == 0)
    {
        return vtsn->stat_active;
    }
    else if (ngx_http_vhost_traffic_status_node_member_cmp(member, "ConnectCurrent") == 0)
    {
        return vtsn->stat_active;
    }
    else if (ngx_http_vhost_traffic_status_node_member_cmp(member, "headerMsec") == 0)
    {
        return vtsn->stat_upstream.header_time;
    }
	//holyzone end
#if (NGX_HTTP_CACHE)
