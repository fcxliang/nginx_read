...
// add by lxl
ngx_int_t
ngx_http_vhost_traffic_status_filter_merge(ngx_pool_t *pool, ngx_array_t **keys_merged, ngx_array_t *ckeys, ngx_array_t *pkeys)
{
    ngx_uint_t nelts_sum = ckeys->nelts + pkeys->nelts;
    ngx_array_t *keys_all;

    size_t unit_size = sizeof(ngx_http_vhost_traffic_status_filter_t);

    keys_all = ngx_array_create(pool, nelts_sum, unit_size);
    if (keys_all == NULL) {
        return NGX_ERROR;
    }
    ngx_memcpy(keys_all->elts, ckeys->elts, ckeys->nelts*unit_size);
    keys_all->nelts += ckeys->nelts;

    ngx_memcpy(keys_all->elts+ckeys->nelts*unit_size, pkeys->elts, pkeys->nelts*unit_size);

    keys_all->nelts += pkeys->nelts;

    *keys_merged = keys_all;
