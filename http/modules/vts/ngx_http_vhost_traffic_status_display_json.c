    ...
    u_char *buf)
{
    ...
    ngx_uint_t rc;			//holyzone
    ngx_atomic_int_t hs,hsf,sr;			//holyzone
	...
	// holyzone
    rc = ngx_cycle->reload_counter;
    hs = *handshaked_counter;
    hsf = *handshaked_fail_counter;
    sr = *session_reuses_counter;
	// holyzone end
    ...
    buf = ngx_sprintf(buf, NGX_HTTP_VHOST_TRAFFIC_STATUS_JSON_FMT_MAIN, &ngx_cycle->hostname,
                      NGX_HTTP_VTS_MODULE_VERSION, NGINX_VERSION, rc , hs, hsf, sr, vtscf->start_msec,
    ...
    ngx_http_vhost_traffic_status_node_t *vtsn)
{
    ...
    tmp = *key;
    // 取得filter name或者是filter key， 第二个位置
    ...
    rc = ngx_http_vhost_traffic_status_escape_json_pool(r->pool, &dst, &tmp); //做escape符号替换dst输出
    if (rc != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    ...
    u_char *buf, ngx_rbtree_node_t *node)
{
    ...
    if (node != ctx->rbtree->sentinel) {
        ...
        if (vtsn->stat_upstream.type == NGX_HTTP_VHOST_TRAFFIC_STATUS_UPSTREAM_NO) {
            key.data = vtsn->data; //拿到key
            key.len = vtsn->len;

            ovtsn = vtscf->stats; //计数结构
            ...
            vtscf->stats.stat_active += vtsn->stat_active; //holyzone
    ...
    u_char *buf, ngx_http_vhost_traffic_status_node_t *vtsn)
{
    ...
    (void) ngx_http_vhost_traffic_status_node_position_key(&key, 2); //取第三个参数，key
    ...
    if (filter_keys != NULL && rc == NGX_OK) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (filter_nodes != NULL && rc == NGX_OK) {
                ...
                for (j = 0; j < filter_nodes->nelts; j++) { //filter key下每个node的计数
     ...
     )
{
    ...
    if (vtsn != NULL) {
        buf = ngx_sprintf(buf, NGX_HTTP_VHOST_TRAFFIC_STATUS_JSON_FMT_UPSTREAM,
                &key, &vtsn->domain, //holyzone domain
                ...
                ngx_http_vhost_traffic_status_display_get_histogram_bucket_counters(r,
                    &vtsn->stat_upstream.response_buckets),
				// holyzone
              	//vtsn->stat_upstream.header_time_counter,
              	ngx_http_vhost_traffic_status_node_time_queue_average(
                	&vtsn->stat_upstream.header_times, vtscf->average_method,
                    vtscf->average_period),
              	ngx_http_vhost_traffic_status_display_get_time_queue_times(r,
        			&vtsn->stat_upstream.header_times),
              	ngx_http_vhost_traffic_status_display_get_time_queue_msecs(r,
					&vtsn->stat_upstream.header_times),
              	ngx_http_vhost_traffic_status_display_get_histogram_bucket_msecs(r,
					&vtsn->stat_upstream.header_buckets),
              	ngx_http_vhost_traffic_status_display_get_histogram_bucket_counters(r,
					&vtsn->stat_upstream.header_buckets),
			  	//holyzone end
              ...
              //vtsn->stat_header_time_counter_oc, //holyzone
                );

    } else {
        buf = ngx_sprintf(buf, NGX_HTTP_VHOST_TRAFFIC_STATUS_JSON_FMT_UPSTREAM,
				&key, (u_char *) "",(ngx_atomic_uint_t) 0,  //holyzone modified
				(ngx_atomic_uint_t) 0,
                ...
                (u_char *) "", (u_char *) "",
                (ngx_atomic_uint_t) 0,
                (ngx_msec_t) 0,					//holyzone
                (u_char *) "", (u_char *) "",	//holyzone
                (u_char *) "", (u_char *) "",	//holyzone
              	(ngx_msec_t) 0,
              	(u_char *) "", (u_char *) "",
              	(u_char *) "", (u_char *) "",
    ...
    u_char *buf)
{
    ...
    buf = ngx_http_vhost_traffic_status_display_set_main(r, buf);  //基本信息，pid我们也可以加这里

    /* serverZones */
    buf = ngx_sprintf(buf, NGX_HTTP_VHOST_TRAFFIC_STATUS_JSON_FMT_SERVER_S);

    //遍历node为根的树，打印每个节点的计数，汇总总计数
    buf = ngx_http_vhost_traffic_status_display_set_server(r, buf, node); //每个NGX_HTTP_VHOST_TRAFFIC_STATUS_UPSTREAM_NO
    //打印总的计数，都放在sum_key下
    ...
    buf = ngx_http_vhost_traffic_status_display_set_filter(r, buf, node); //每个NGX_HTTP_VHOST_TRAFFIC_STATUS_UPSTREAM_FG
