...
// 将dst根据type的不同转成 NO/UG/CC/FG+等分隔符+dst的形式，通过buf返回
ngx_int_t
    ...
    ngx_str_t *buf, ngx_str_t *dst, unsigned type)
{
    ...
    len = ngx_strlen(ngx_http_vhost_traffic_status_group_to_string(type)); //将类型转为字符串计算长度

    buf->len = len + sizeof("@") - 1 + dst->len; //"UG@dst"
    ...
    p = ngx_cpymem(p, ngx_http_vhost_traffic_status_group_to_string(type), len); //NO UG CC FG等
    *p++ = NGX_HTTP_VHOST_TRAFFIC_STATUS_KEY_SEPARATOR; //加上一个单元分隔符
    p = ngx_cpymem(p, dst->data, dst->len); //加上dst内容

    return NGX_OK;
}

// 找到第pos个key，每个key使用分隔符分割
ngx_int_t
    ...
    ngx_str_t *key, unsigned type, uint32_t key_hash)
{
    ...
    if (hash == 0) {
        hash = ngx_crc32_short(key->data, key->len); //没有hash才需要计算
    }
    //计数节点缓存，缓存每个类型单一值
...
ngx_http_vhost_traffic_status_find_lru(ngx_http_request_t *r)
{
    ...
    if (shm_info->filter_used_node >= ctx->filter_max_node) {
        // 当过滤节点超过最大过滤节点的时候，找一个同一个filterkey最早的过滤节点
    ...
    ngx_http_vhost_traffic_status_node_t *vtsn)
{
	...
	//holyzone
    //vtsn->stat_upstream.header_time_counter = 0;
    vtsn->stat_upstream.header_time = 0;
    ngx_http_vhost_traffic_status_node_time_queue_init(&vtsn->stat_upstream.header_times);
    ngx_http_vhost_traffic_status_node_histogram_bucket_init(r,
    ...
    ngx_http_vhost_traffic_status_node_t *vtsn)
{
    ...
    ms = ngx_http_vhost_traffic_status_request_time(r); //当前毫秒
    ngx_http_vhost_traffic_status_node_update(r, vtsn, ms); //增加计数
    ...
    ngx_http_vhost_traffic_status_add_oc((&ovtsn), vtsn); //处理计数值溢出
}
    ...
    ngx_http_vhost_traffic_status_node_t *vtsn, ngx_msec_int_t ms)
{
    ...
    ngx_http_vhost_traffic_status_add_rc(status, vtsn);  // http增加各个code的计数值
    ...
    ngx_str_t *member)
{
	...
	// holyzone
    else if (ngx_http_vhost_traffic_status_node_member_cmp(member, "active") == 0)
    {
        return vtsn->stat_active;
    }
	// holyzone end
