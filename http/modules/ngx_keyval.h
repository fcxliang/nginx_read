...
typedef struct {
  ngx_array_t *indexes; //初始申请了4个，可存放4个ngx_array_t
  ngx_array_t *variables; //初始申请了4个，为什么？？
  ngx_array_t *zones;
} ngx_keyval_conf_t;
...
typedef struct {
  ngx_array_t *indexes; //key中所有的变量索引
  ngx_str_t key_string; //key中除了变量剩余的部分，没有变量的话就是完整的key
  ngx_str_t variable; //value变量，就是变量的名称
