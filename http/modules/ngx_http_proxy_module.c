...
static ngx_command_t  ngx_http_proxy_commands[] = {
	...
	{ ngx_string("proxy_snat_pool"),
		...
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_proxy_loc_conf_t, upstream.snat_pool),
		NULL },
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    if (ngx_http_upstream_create(r) != NGX_OK) { // <----------- upstream初始化
    ...
    ngx_http_set_ctx(r, ctx, ngx_http_proxy_module); //设置proxy模块的ctx
    ...
    rc = ngx_http_read_client_request_body(r, ngx_http_upstream_init);  //ngx_http_upstream_init 启动upstream机制
...
ngx_http_proxy_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->upstream.snat_pool = NGX_CONF_UNSET_PTR;  //add by lxl
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    //snatpool,by lxl    
    ngx_conf_merge_ptr_value(conf->upstream.snat_pool,
                              prev->upstream.snat_pool, NULL);
...
static char *
ngx_http_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)  //proxy_Pass指令
{
    ...
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module); //他的上一级location指令

    clcf->handler = ngx_http_proxy_handler; //设置proxy处理函数
